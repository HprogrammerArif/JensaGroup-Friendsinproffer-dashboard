...
const CleanerRow = ({ cleaner, onEdit, onDelete, onToggleActive }) => {
  const name = cleaner.user_profile?.full_name || "—";
  const phone = cleaner.phone_or_email || "—";
  const areas = cleaner.assined_area_data?.map((a) => a.name).join(", ") || "—";
  const tasks = cleaner.total_tasked_compelted ?? 0;

  return (
    <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
      <td className="px-4 py-3 text-sm text-slate-500">{cleaner.member_no}</td>
...
const CleanerCard = ({ cleaner, onEdit, onDelete, onToggleActive }) => {
  const name = cleaner.user_profile?.full_name || "—";
  const phone = cleaner.phone_or_email || "—";
  const areas = cleaner.assined_area_data?.map((a) => a.name).join(", ") || "—";
  const tasks = cleaner.total_tasked_compelted ?? 0;
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-2 flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-slate-400">#{cleaner.member_no}</p>
...
const CleanersTable = ({ cleaners, onEdit, onDelete }) => {
